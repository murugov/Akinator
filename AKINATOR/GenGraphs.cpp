...
AkinErr_t GenHTML()
{
    FILE *GraphsHTML = fopen("AKINATOR/ReportFiles/GraphsHTML.html", "w");
    if (IS_BAD_PTR(GraphsHTML)) return AKIN_ERROR;

    fprintf(GraphsHTML, "<!DOCTYPE html>\n"
                        "<html>\n"
                        "<head>\n"
                        "\t<title>AkinatorDump</title>\n"
                        "\t<style>\n"
                        "\t\tbody {\n"
                        "\t\t\tfont-family: Arial;\n"
                        "\t\t\tdisplay: flex;\n"
                        ...
                        "</head>\n"
                        "<body>\n"
                        "\t<h1 text-align: center;>AkinatorDump</h1>\n");
    ...
    for (size_t i = 0; i < number_graph; ++i)
    {
        ...
        fprintf(GraphsHTML, "\t\t\t<h4>graph%zu/akin.dot.png:</h4>\n", i);
        fprintf(GraphsHTML, "\t\t\t<img src=\"graphs/graph%zu/akin.dot.png\">\n", i);
...
AkinErr_t GenGraphs(tree_t *tree, const char *func)
{
    ...
    char file_path[64] = {0};
    snprintf(file_path, sizeof(file_path), "AKINATOR/ReportFiles/graphs/graph%zu/akin.dot", number_graph);

    FILE *graph = fopen(file_path, "w");
    if (IS_BAD_PTR(graph)) return AKIN_ERROR;

    if (GenDot(graph, tree, func))  return AKIN_ERROR;
...
AkinErr_t CreateNode(FILE *src, node_t *node, int node_id)
{
    ON_DEBUG(
        if (IS_BAD_PTR(src))  return AKIN_ERROR;
        if (IS_BAD_PTR(node)) return AKIN_ERROR;
    )
...
AkinErr_t GenDot(FILE *src, tree_t *tree, const char *func)
{
    ...
    if (tree->root == NULL) { fprintf(src, "}"); return AKIN_SUCCESS; }
    ...
    fprintf(src, "\tn%zu [shape=record, label=\"{ptr = %p | parent = %p | data = Is it %s? | {<left> left = %p | <right> right = %p}}\", fillcolor=\"#87CEEB\", color=\"black\"]\n", 
    ...
    while (stk_ret.size > 0)
    {
        ...
        if (current_node->left != NULL)
        {
            ...
            fprintf(src, "\tn%zu:left -> n%zu\n [label=\"yes\", color=\"#008000\", fontcolor=\"#008000\"]", current_id, number_nodes);
        ...
        if (current_node->right != NULL)
        {
            ...
            fprintf(src, "\tn%zu:right -> n%zu [label=\"no\", color=\"#B22222\", fontcolor=\"#B22222\"]\n", current_id, number_nodes);
